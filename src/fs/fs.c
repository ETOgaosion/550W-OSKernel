...
//     uint32_t* table = k_malloc(fat.bpb.bytes_per_sec);
//     int ret = sbi_sd_read((uint32_t)table,1,fat_cluster2fatsec(cluster));
//     if(ret){
//         printk("> [FAT32 traverse] fat sector read error!\n");
//         return 0;
//     }
...
//         return 0;
//     if(next == FAT_BAD)
//         printk("> [FAT32 traverse] current cluster %d bad status!\n",cluster);
...
uint32_t fat32_fcluster2size(uint32_t first) {
    ...
    do {
        if (cur_fat_sec == 0xffffffff || fat_cluster2fatsec(first) != cur_fat_sec) {
            ...
            k_sd_read((char *)ftable, &cur_fat_sec, 1);
            // printk("read %s \n",buffer[0]->data);
        }
...
void *read_whole_dir(uint32_t first, uint32_t size) {
    if (first < 2) {
        if (size != 0) {
            return k_malloc(((size + fat.bytes_per_cluster - 1) / fat.bytes_per_cluster) * fat.bytes_per_cluster);
        }
        printk("[ERROR] read empty file!\n");
        return NULL;
    }
    uint32_t ftable[fat.bpb.bytes_per_sec / sizeof(uint32_t)];
    if (size == 0) {
    ...
    // printk("size = %d, malloc %d sec bytes\n", size, (size / fat.bpb.bytes_per_sec + 1) * fat.bpb.bytes_per_sec);
    uint8_t *data = k_malloc(((size + fat.bytes_per_cluster - 1) / fat.bytes_per_cluster) * fat.bytes_per_cluster);
    // printk("[debug] file size = %u, kalloc %u\n",size,((size + fat.bytes_per_cluster - 1) / fat.bytes_per_cluster) * fat.bytes_per_cluster);
    int cnt = 0;
    uint32_t cur_fat_sec = 0xffffffff;
    do {
        if (cur_fat_sec == 0xffffffff || fat_cluster2fatsec(first) != cur_fat_sec) {
            ...
            //  fread(ftable,fat.bpb.bytes_per_sec,1,fp);
            k_sd_read((char *)ftable, &cur_fat_sec, 1);
            // printk("[debug] read cur sec %x, first %x\n",cur_fat_sec,first);
        }
        // TODO
        ...
        for (int i = 0; i < fat.bpb.sec_per_cluster; i++) {
            ...
            // printk("[debug] read sec %d in cluster %x\n",sec_num,first);
            k_sd_read((char *)(&data[cnt * fat.bytes_per_cluster + i * fat.bpb.bytes_per_sec]), &sec_num, 1);
        }
        first = ftable[fat_cluster2fatoff(first)] & FAT_MASK;
        cnt++;
    } while (first < FAT_MAX && first > 2);
...
int write_whole_dir(uint32_t first, void *data, uint32_t write) {
    ...
    do {
        ...
        for (int i = 0; i < fat.bpb.sec_per_cluster; i++) {
            uint32_t sec_num = fat_cluster2sec(first) + i;
            // printk("[debug] write sec %d in cluster %x\n",sec_num,first);
            k_sd_write((char *)(&((uint8_t *)data)[cnt * fat.bytes_per_cluster + i * fat.bpb.bytes_per_sec]), &sec_num, 1);
        }
        first = ftable[fat_cluster2fatoff(first)] & FAT_MASK;
        cnt++;
    } while (first < FAT_MAX && first > 2);
...
uint32_t alloc_cluster(uint32_t first) {
    ...
    if (first != 0) {
        ...
        do {
            if (cur_fat_sec == 0xffffffff || fat_cluster2fatsec(first) != cur_fat_sec) {
                ...
                k_sd_read((char *)ftable1, &cur_fat_sec, 1);
            }
    ...
    while (i < fat.bpb.sec_per_fat) {
        ...
        k_sd_read((char *)ftable2, &base, 1);
        int j = 0;
        for (j = 0; j < max; j++) {
            // printk("[debug] fat entry(%d,%d) :%x\n",i,j,ftable2[j] & FAT_MASK);
            if ((ftable2[j] & FAT_MASK) == 0) {
                ftable2[j] = FAT_MAX;
                // printk("[debug] assigned value %x\n",ftable2[j]);
                // new cluster num
...
int fat32_new_dentry(dentry_t *entry, uint32_t flags, int len, char *name) {
    // printk("[debug] openat add dentry! len = %d, name %s\n",len,name);
...
int alloc_dentry(dentry_t **dtable, char *name, mode_t flags, mode_t mode, dir_info_t *now) {
    ...
    if (i >= size) {
        // printk("[debug] openat alloc new cluster!\n");
...
int fat32_init() {
    ...
    // read BOOT_SEC
    uint8_t *boot = k_malloc(PAGE_SIZE);
    uint32_t boot_sec = 0;

    k_sd_read((char *)boot, &boot_sec, 1);
    k_sd_write((char *)boot, &boot_sec, 1);
    ...
    printk("> [FAT32 init] fat_totl: %d\n", fat.bpb.totl_sec);
    printk("> [FAT32 init] fat_byts_per_clus: %d\n", fat.bytes_per_cluster);
    printk("> [FAT32 init] fat_first_data_sec: %d\n", fat.first_data_sec);
    printk("> [FAT32 init] fat_data_cluster_cnt: %d\n", fat.data_cluster_cnt);
    printk("> [FAT32 init] fat.next_free_cluster: %d\n", fat.next_free_cluster);
    ...
    root_dir.name[0] = '/';
    root_dir.name[1] = '\0';
    printk("root dir begin at cluster %d, size = %d\n", root_dir.first_cluster);
    ...
    // if(file_txt)
    //     printk("[test] 1. read openat success! name %s\n",file_txt->name);
    // int read_len = sys_read(read_fd,name,256);
    // // printk("[test] 2. read %d bytes : %s\n",read_len,name);
    ...
    // printk("[test] getcwd %s",name);

    // // [dup] ok
    // int dup_fd = sys_dup(STDOUT);
    // printk("[test] dup fd %d\n",dup_fd);
    ...
    //     printk("[test] write success!\n");
    // }else{
    //     printk("[test] write fail? strlen = %d, write len = %d",str_len,sys_write(STDOUT, wstr, str_len));
    // }
    ...
    // int disk_len;
    // int elf_len;
    // int ret = k_load_file("test_virtio.elf",&disk_data,&disk_len);
    // if(!ret)
    //     ret = get_elf_file("virtio",&elf_data,&elf_len);
    // else
    //     ret = 0;
    // if(disk_len==elf_len && ret !=0){
    //     printk("[debug] get same shell.elf length %d!\n",disk_len);
    //     int len = disk_len;
    //     while(len-- > 0){
    //         if(elf_data[len]!=disk_data[len]){
    //             printk("[debug] file not same at offset %d, elf %u, disk %u\n",len,elf_data[len],disk_data[len]);
    //             break;
    //         }
    //     }
    //     printk("[debug] cmp finish!\n");
    // }else{
    //     printk("[debug] get diff shell.elf length! elf len %d, disk len %d\n",elf_len,disk_len);
    // }
    // kfree
    return 0;
}
...
int sys_mkdirat(int dirfd, const char *path_0, mode_t mode) {
    ...
    // printk("[fat32] openat create file with name %s!\n",name);
...
int sys_openat(int dirfd, const char *filename, mode_t flags, mode_t mode) {
    ...
    if (offset >= 0) {
        if (!(((dtable[offset].sn.attr & ATTR_DIRECTORY) == 0) ^ ((flags & O_DIRECTORY) == 0))) {
            // printk("[debug] openat file type right!\n");
            fat32_dentry2fd(&dtable[offset], file, flags, mode, name);
            return ret;
        }
    } else if (offset < 0 && (flags & O_CREATE)) {
        // printk("[debug] openat create file with name %s!\n",name);
...
int sys_unlinkat(int dirfd, const char *path_0, mode_t flags) {
    ...
    offset = fat32_name2offset(name, dtable);
    if (offset < 0) {
        // printk("[debug] : testopen not found!\n");
        // free dtable
        return -1;
    }
    if (!(((dtable[offset].sn.attr & ATTR_DIRECTORY) == 0) ^ ((flags & AT_REMOVEDIR) == 0))) {
        // printk("[debug] destroy %s at dir %s, offset %d\n",name,path,offset);
...
ssize_t sys_read(int fd, char *buf, size_t count) {
    ...
    if (file->is_pipe_read) {
        pipe_t *p = &pipe_table[file->pip_num];
        return read_ring_buffer(&p->rbuf, (uint8_t *)buf, count);
    }
...
ssize_t sys_write(int fd, const char *buf, size_t count) {
    ...
    if (file->is_pipe_write) {
        pipe_t *p = &pipe_table[file->pip_num];
        return write_ring_buffer(&p->rbuf, (uint8_t *)buf, count);
    }

    int new = (file->pos + count) / fat.bytes_per_cluster - file->size / fat.bytes_per_cluster; // need more cluster?
    while (new > 0) {
        alloc_cluster(file->first_cluster);
        new --;
    }
...
int k_load_file(const char *name, uint8_t **bin, int *len) {
    ...
    if (offset < 0) {
        // printk("[debug] can't load, no such file!\n");
        return -1;
    }
    ...
    *len = dtable[offset].sn.file_size;
    if (*len == 0) {
        // printk("[debug] can't load, empty file!\n");
        return -1;
    }
