...
uintptr_t shm_page_get(int key)
{
    ...
    for(int i=0; i<shm_num; i++)
    {
        ...
        {
            map(shm_all[i].uva, kva2pa(shm_all[i].kva), 
                (pa2kva(current_running->pgdir << 12)));
            shm_all[i].pro_num++;
    ...
    allocpid = current_running->pid;
    if(num==-1) num=shm_num;
    shm_all[num].key = key;
    shm_all[num].uva = alloc_newva();
    shm_all[num].kva = alloc_page_helper(shm_all[num].uva, 
        (pa2kva(current_running->pgdir << 12)));
...
void fork_pgtable(PTE* dest_pgdir, PTE* src_pgdir)
{
    ...
    {
        if (src_pgdir[vpn2] % 2 == 1) {
            PTE *pmd = (PTE *)get_kva(src_pgdir[vpn2]);
                ...
                if(pmd[vpn1] % 2 == 1)
                {
                    ...
                    for(uint64_t vpn0=0; vpn0<512; vpn0++)
                    {
                        if((pld[vpn0] % 2) == 1)
                        {
                            ...
                            if(va!=sva && ((va & (1<<39))==0))
                            {
                                uint64_t pa_kva = get_kva(pld[vpn0]);
                                //src pgdir
                                pld[vpn0] = ((pld[vpn0] >> 3) <<3) | 3;
                                //printk("vpn: %d, %d, %d, va:0x%x, pa:0x%x\n", vpn2,vpn1,vpn0, va<<NORMAL_PAGE_SHIFT, kva2pa(pa_kva));
                                
                                map(va, kva2pa(pa_kva), dest_pgdir);
                                //dest pgdir
                                PTE* spmd = (PTE *)get_kva(dest_pgdir[vpn2]);
                                PTE* spld = (PTE *)get_kva(spmd[vpn1]);
...
void fork_page_helper(uintptr_t va, PTE* destpgdir, PTE* srcpgdir)
{
    ...
    if (srcpgdir[vpn2] == 0)
        assert(0);
    PTE *pmd = (PTE *)get_kva(srcpgdir[vpn2]);
    if(pmd[vpn1] == 0)
        assert(0);
    PTE *pld = (PTE*)get_kva(pmd[vpn1]);
    uint64_t src_kva = get_kva(pld[vpn0]);
...
{
    for (uint64_t kva = 0x50000000lu; kva < 0x50400000lu;
         kva += 0x200000lu) {
