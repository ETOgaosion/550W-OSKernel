...
void init_pcb_i(char *name, char *cmd, int pcb_i, task_type_t type, int pid, int tid, int uid, int father_pid, uint8_t core_mask, sigaction_t *sig) {
    ...
    k_memcpy(target->cmd, cmd, NUM_MAX_PCB_CMD);
    target->in_use = TRUE;
    target->pid = pid;
    target->tid = tid;
    if (type == USER_THREAD) {
...
uintptr_t init_user_stack(pcb_t *new_pcb, ptr_t *user_stack_kva, ptr_t *user_stack, int argc, const char *argv[], int envpc, const char *envp[], const char *file_name, bool dynamic) {
    ...
    *((uintptr_t *)kargv_pointer + envpc) = 0;
    ...
    // k_memcpy(kargv_pointer, aux_vec, sizeof(aux_elem_t) * (AUX_CNT + 1));
...
void check_itimers() {
    ...
    if (k_time_cmp_utime_0(&(*current_running)->real_itime.it_value) < 0) {
        k_signal_send_signal(SIGALRM, *current_running);
        time_up = true;
    }
    if (k_time_cmp_utime_0(&(*current_running)->virt_itime.it_value) < 0) {
        k_signal_send_signal(SIGVTALRM, *current_running);
        time_up = true;
    }
    if (k_time_cmp_utime_0(&(*current_running)->prof_itime.it_value) < 0) {
        k_signal_send_signal(SIGPROF, *current_running);
        time_up = true;
    }
    if (time_up) {
...
long clone(unsigned long flags, void *stack, pid_t *parent_tid, void *tls, pid_t *child_tid) {
    ...
    k_memcpy(name, (*current_running)->name, name_len);
...
int kill(pid_t pid, int exit_status) {
    ...
    k_signal_free_sig_table(target->sigactions);
    if (target->list.next) {
...
void k_pcb_stat(int pid, char *buf) {
    ...
    // pid
    k_ltoa(pid, str, 10);
    k_memcpy(buf, str, k_strlen(str));
    k_strcat(buf, " ");
    // comm
    k_strcat(buf, pcb[pid].name);
    k_strcat(buf, " ");
    // stat
    ...
    k_strcat(buf, "0 ");
    // uname
    k_strcat(buf, uname_550w.sysname);
    // stime
...
long sys_tkill(pid_t pid, int sig) {
    ...
    if (pcb[pid].father_pid) {
        k_signal_send_signal(SIGCHLD, &pcb[pcb[pid].father_pid]);
    }
    k_signal_send_signal(sig, &pcb[pid]);
    return 0;
}
...
long sys_wait4(pid_t pid, int *stat_addr, int options, rusage_t *ru) {
    ...
    if (target->in_use) {
        ...
        if (ru) {
            k_memcpy(ru, &target->resources, sizeof(rusage_t));
        }
    }
    return wait_pid;
}
...
long sys_getrusage(int who, rusage_t *ru) {
    ...
    k_memcpy(ru, &pcb[who].resources, sizeof(rusage_t));
    return 0;
}
...
long sys_prctl(int option, unsigned long arg2, unsigned long arg3, unsigned long arg4, unsigned long arg5) {
    switch (option) {
    case PR_SET_NAME:
        k_strncpy((*current_running)->name, arg2, k_strlen((*current_running)->name));
        break;
    case PR_GET_NAME:
        k_strncpy(arg2, (*current_running)->name, k_strlen((*current_running)->name));
        break;
...
long sys_capget(cap_user_header_t *header, cap_user_data_t *dataptr) {
    k_memcpy(dataptr, pcb[header->pid].cap, 2 * sizeof(cap_user_data_t));
    return 0;
}

long sys_capset(cap_user_header_t *header, const cap_user_data_t *data) {
    k_memcpy(pcb[header->pid].cap, data, 2 * sizeof(cap_user_data_t));
    return 0;
}