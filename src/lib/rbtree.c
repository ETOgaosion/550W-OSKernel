 ...
 */
RBNode *search(RBTree x, KeyType key) {
    if (x == NULL || x->key == key) {
        return x;
    }
 ...
 */
RBNode *iterative_search(RBTree x, KeyType key) {
 ...
 */
RBNode *rbtree_successor(RBTree x) {
 ...
 */
RBNode *rbtree_predecessor(RBTree x) {
 ...
 *     right 是右孩子。
 */
RBNode *create_rbtree_node(RBNode *p, KeyType key, RBNode *parent, RBNode *left, RBNode *right) {
    p->key = key;
    p->left = left;
    p->right = right;
    p->parent = parent;
 ...
 *                1，表示该节点是它的父结点的右孩子。
 */
void rbtree_print(RBTree tree, KeyType key, int direction) {
    if (tree != NULL) {
        if (direction == 0) { // tree是根节点
            k_print("%2d(B) is root\n", tree->key);
        } else { // tree是分支节点
            k_print("%2d(%s) is %2d's %6s child\n", tree->key, rb_is_red(tree) ? "R" : "B", key, direction == 1 ? "right" : "left");
        }
