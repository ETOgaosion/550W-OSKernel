...
static inline int ring_buffer_full() {
    return (((ring_buffer_tail + 1) % BUFFER_LENGTH) == ring_buffer_head);
}
...
void recv_thread(void *arg) {
    ...
    for (;;) {
        ...
        sys_move_cursor(1, position);
        printf("[%c-recv] from %c: %ld \t from %c: %ld\n", id, other_id[0], bytes[other_id[0] - 'a'], other_id[1], bytes[other_id[1] - 'a']);
        sys_sleep(1);
    }
}
...
void send_thread(void *arg) {
    ...
    for (;;) {
        ...
        sys_move_cursor(1, position);
        printf("[%c-send] to %c: %ld \t to %c: %ld\n", id, other_id[0], bytes[0], other_id[1], bytes[1]);
        sys_sleep(1);
    }
}
