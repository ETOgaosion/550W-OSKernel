...
void extend(Records *self) {
    ...
    self->record = (char ***)realloc(self->record, sizeof(char **) * self->capicity);
    self->record[0] = (char **)realloc(self->record[0], sizeof(char *) * 3 * self->capicity);
...
char *variable_name(const char *buffer) {
    ...
    int l = strlen(var);
    ret = (char *)malloc(sizeof(char) * (l + 1)); // strlen(var) + 1(this is for '\0')
    strcpy(ret, var);
    return ret;
}

FILE *open_file_with_suffix(const char *file_name, const char *suffix) {
    char *file_name_buf = (char *)malloc(sizeof(char) * (strlen(file_name) + strlen(suffix) + 1));
